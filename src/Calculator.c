...
{
	const int sum = Converter_roman_to_arabic(numeral1) + Converter_roman_to_arabic(numeral2);
	Converter_arabic_to_roman(buffer, sum);
}

void Calculator_subtract(char * buffer, char * numeral1, char * numeral2)
{
	const int difference = Converter_roman_to_arabic(numeral1) - Converter_roman_to_arabic(numeral2);
